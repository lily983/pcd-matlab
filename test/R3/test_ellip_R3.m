...
for i=1:N
    ...
    rand(3,1)*sizeScale, quat, [20, 20]});

    quat=rand(1,4);
    quat = quat/norm(quat);

    s2 = SuperQuadrics({(rand(1,3)+0.1)*10*sizeScale,  [1,1], [0, 0]...
       (rand(3,1)+0.3)*10*sizeScale, quat,[20,20]});
    ...
    Sigma = Sigma.*0.08*sizeScale^2;
    
%     v =  4*pi/3*(s1.a(1)+s2.a(1))^3 ;
%     
%     try
%        [pdf_max_x, x_max] = max_contact_probability_pure_translation_superquadrics(s2.tc, Sigma, s1, s2, flag);
...
%         exact_prob_x=0;
    ...
    j=j+2; result(j:j+2, i) = s1.a;
    j=j+3; result(j:j+2, i) = s1.tc;
    j=j+3; result(j:j+3, i) =s1.q;
    j=j+5; result(j:j+2, i) = s2.a;
    j=j+3; result(j:j+2, i) = s2.tc;
