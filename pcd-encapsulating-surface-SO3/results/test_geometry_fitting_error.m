...
for i = 1:N_trial
    ...
    [sq_shape, error_pt_surf(i), sq_fitting_time(i)] = sq_fitting([], poly);
    ...
    % Fit ellipsoid model
    [ellip_shape, error_pt_surf_ellip(i), ellip_fitting_time(i)] = ellip_fitting([], poly);
...
disp(['Computation time SQ fit Polyhedra: ',...
    num2str( mean(sq_fitting_time) )]);
disp(['Computation time ellip fit Polyhedra: ',...
    num2str( mean(ellip_fitting_time) )]);
