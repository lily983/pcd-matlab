...
sq = SuperQuadrics({axes, eps, [0,0], zeros(3,1), rotm2quat(mu_SO3), [20,20]});

%% Visualize the obj 
figure; hold on; axis equal

for i=1:size(R,3)
    sq_i = SuperQuadrics({sq.a, sq.eps, [0,0], zeros(3,1), rotm2quat(R(:,:,i)), [20,20]});
    sq_i.PlotShape('b', 0.05,0.3);
end
sq.PlotShape('y', 0.4,0.2);
%% Generate surface points of sq at errorneous orientations
sq_points_cell = {};

for i=1:size(R,3)
    sq_i = SuperQuadrics({sq.a, sq.eps, [0,0], zeros(3,1), rotm2quat(R(:,:,i)), [20,20]});
    sq_points_cell{i} = sq_i.GetPoints();
end

%% Range of k values
k_values = 1.0:0.05:2;   % from 1.0 to 1.4 with step 0.05
...
for ki = 1:num_k
    ...
    x_new_method{1}  = method5(sq, R, k);
    x_new_method{2}  = method6(sq, k);
    x_new_method{3}  = method9(sq, R, k);
    x_new_method{4} = method10(sq, R, k);

    % --- Compute scores ---
    for m = 1:num_methods
        fitting_scores(m, ki) = metric_fitting_multi_Strue(x_new_method{m}, sq_points_cell);
        deficit_scores(m, ki) = metric_deficit_multi_Strue(x_new_method{m}, sq_points_cell);
    end
end

%% ---- Labels + quick sanity ----
method_names = "Method " + ["5", "6", "9", "10"];
