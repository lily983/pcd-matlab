function d = test_My_CCMPC_2_lcc_center(obstacles, params)
d.p = params;
...
for k=2:d.p.N_NMPC+1
    ...
    for o = 1:length(obstacles)
        ...
        if obstacle.shape == "circle"
            ...
            con = [con, norm_p_io - b_io >= safety_distance - slack_collision, slack_collision >= 0];
        ...
        elseif obstacle.shape == "ellipse"
            ...
            Sigma_io_tilde = Omega_sqrt * (Sigma_i(:,:,k)+R_o' * obstacle.cov * R_o) * Omega_sqrt';

            % Probabilistic constraint
            slack_collision = sdpvar(1);
            safety_distance = erf_inv_delta * (2*a_io' * Sigma_io_tilde * a_io)^0.5;
            
%             con = [con, p_io'*Omega_sqrt*p_io  - b_io >= safety_distance - slack_collision, slack_collision >= 0];
             con = [con, p_io'*Omega_sqrt*p_io  - b_io >= safety_distance - slack_collision, slack_collision >= 0];
            obj = obj + 5000*slack_collision; % High penalty for violation
        end
