function d = test_CCMPC(obstacles, params)
d.p = params;
...
function [cost] = objective_function(z, d, obstacles, t)
    ...
    for k = 1:N
        ...
        % Add collision avoidance penalties
        ...
        for o = 1:length(d.c.obstacles)
            ...
            % Safety distance with probability
            quadratic_term = 2*a_io'*Sigma_io*a_io;
            safety_distance = d.c.erf_inv_delta * sqrt(quadratic_term);
            
            % Distance check
            actual_distance = norm(p_io) - b_io;
            if actual_distance < safety_distance
                cost = cost + 5000 * (safety_distance - actual_distance)^2;
            end
        end
    end
...
function [c, ceq] = nonlinear_constraints(z, d, obstacles, t)
    ...
    % For simplicity, we'll implement these as soft constraints in the objective function
    c = [];
end
