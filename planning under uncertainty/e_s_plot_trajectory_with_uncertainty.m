function e_s_plot_trajectory_with_uncertainty(d, obstacles)
    % Create a new figure
    ...
    for i = 1:length(obstacles)
        ...
        % Check obstacle shape and plot accordingly
        if strcmpi(obstacle.shape, "circle")
        ...
        elseif strcmpi(obstacle.shape, "ellipse")
            % Plot elliptical obstacle
            a = obstacle.semi_axes(1); % Semi-major axis
            b = obstacle.semi_axes(2); % Semi-minor axis
            ...
            for j = 1:length(theta)
                % Create points for an ellipse centered at origin
                x_temp = a * cos(theta(j));
                y_temp = b * sin(theta(j));
                
                % Rotate the ellipse
                x_rot = x_temp * cos(orientation) - y_temp * sin(orientation);
                y_rot = x_temp * sin(orientation) + y_temp * cos(orientation);
                
                % Translate to the obstacle position
                ellipse_points(1, j) = pos(1) + x_rot;
                ellipse_points(2, j) = pos(2) + y_rot;
            end
            
            % Fill the ellipse
            fill(ellipse_points(1, :), ellipse_points(2, :), 'r', 'FaceAlpha', 0.5);
        end
...
%         ellipse_color = [color_intensity, color_intensity, 0.8];
        ...
        plot_colored_covariance_ellipse(pos, pos_cov, 0.95, 'black');
...
% obstacles = {
%    struct('pos', [1;3], 'cov', diag([0.01, 0.01]), 'shape', "circle", 'radius', 0.3),
%    struct('pos', [4; 3], 'cov', diag([0.001, 0.001]), 'shape', "ellipse", 'semi_axes', [0.1, 0.3], 'orientation', pi/8)
% };
