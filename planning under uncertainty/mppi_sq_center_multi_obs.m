    ...
    Sigma       = diag([1, 1]);  % Noise covariance for controls [v; w]
    Q           = diag([5, 1]);      % State (position) error cost
    R           = diag([0.0001, 0.0001]);  % Control effort cost
    ...
    for t = 1:max_steps
        ...
        [u_opt, best_cost, best_traj, u_seq_updated] = ...
            MPPI_step(x, u_seq, dt, N, num_samples, lambda, Sigma, Q, R, goal, obs_sq_params, robot_sq_param, Sigma_pos, threshold);
...
%                 end
%                 a_io = p_io / norm(p_io);
%                 % Compute the rotated super-ellipse representations for robot and obstacle
%                 R_obs = angle2rotm(obs_param.ang);
%                 x_obs   = R_obs * get_points_from_normal(obs_param, R_obs' * a_io);
...
%                     obstacle_penalty = obstacle_penalty + (threshold - constraint + 1e2);
%                 end
%             end
% 
