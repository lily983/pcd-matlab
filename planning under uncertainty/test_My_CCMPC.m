function d = test_My_CCMPC(obstacles, params)
d.p = params;
...
for t = 1:d.p.sim_steps
    ...
    if t > 1
        ...
        distance_to_goal = norm(current_pos - d.p.goal);
        
        % Display current distance to goal
        fprintf('Time step %d: Distance to goal = %.4f\n', t, distance_to_goal);
        
        % Check if car is close enough to goal
        if distance_to_goal <= d.p.goal_tolerance
            fprintf('Goal reached at time step %d with distance %.4f!\n', t, distance_to_goal);
...
for k = 1:d.p.N_NMPC
    ...
    distance_to_goal = (predicted_pos - d.p.goal)' * (predicted_pos - d.p.goal);
...
for k = 1:d.p.N_NMPC
    ...
    % add stage cost to objective function
    obj = obj + u(:,k)'*d.p.R*u(:,k) + (predicted_pos - d.p.goal)'*d.p.Q_k*(predicted_pos - d.p.goal);
end
