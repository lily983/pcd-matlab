function d = test_hierarchical_CCMPC(obstacles, params)
d.p = params;
...
%                 prob = LCC_SuperEllipse(d.p.sq, obstacle.sq, p_io, Sigma_io, 'center-point-cfc' );
% Transformation matrix for the ellipse
                ...
                if safety_distance - ( a_io' * Omega_sqrt * p_io - b_io) >0 
                    prob = LCC_SuperEllipse(d.p.sq, obstacle.sq, -p_io, Sigma_io, 'tangent-point-cfc' );
                    if prob<d.p.delta
                        d.s.tangent_help(1, end) = 1/2 - 1/2*erf((a_io' * Omega_sqrt * p_io - b_io) / sqrt(max(quadratic_term, 1e-6))) ;
                        d.s.tangent_help(2, end) = prob;
                        d.s.tangent_help(3, end) = d.s.tangent_help(1, end)  - d.s.tangent_help(2, end);
                    end
