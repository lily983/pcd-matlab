function d = test_tangent_CCMPC(obstacles, params)
d.p = params;
...
% copy the obstacles information
d.c.obstacles = obstacles;

disp('%%%%%%%%%%%%%%%%%');
disp('Finished creating NMPC object');
...
%         cost = cost + (x(1:2,k) - d.p.goal(1:2))'*d.p.Q_k*(x(1:2,k) - d.p.goal(1:2));
...
%         end
    ...
    % Terminal cost
    cost = cost + (x(1:2,N+1) - d.p.goal(1:2))'*d.p.Q*(x(1:2,N+1) - d.p.goal(1:2));
end
