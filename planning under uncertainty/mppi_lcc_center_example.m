function mppi_lcc_center_example()
    clc; clear; close all;
    ...
    for t = 1:max_steps
        ...
        [u_opt, best_cost, best_traj, u_seq_updated] = ...
            MPPI_step(x, u_seq, dt, N, num_samples, lambda, Sigma, Q, R, goal, obs, obs_radius, robot_radius, Sigma_pos, threshold);
...
% obs, obs_radius: obstacle center and radius
    ...
    erf_inv_delta = erfinv(1 - 2*threshold);
...
%             end
            ...
            p_io = x_roll(1:2) - obs;
            a_io = p_io./norm(p_io);
            b_io = robot_radius + obs_radius;
%             constraint = erf_inv_delta *sqrt(max( 2*a_io'*Sigma*a_io, 1e-6)) - ( a_io' * p_io - b_io)
            constraint = 0.5 + 0.5*erf((b_io - a_io' * p_io)/sqrt(max( 2*a_io'* (i * Sigma_pos) *a_io, 1e-6)));
            if threshold < constraint
                stage_cost  = threshold - constraint + 1e10;
            end
