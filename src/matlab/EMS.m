...
% emcompass all inliers within the superquadric
if para.IncludeAllInliers == true
    point_inlier = point(:, p >= para.InlierThreshold);
    gamma = max(implicit_function(point_inlier, x)) ^ (x(1) / 2);
    x(3 : 5) = x(3 : 5) .* gamma;
end
...
% ------------------implicit function--------------------------------------
    function [implicit_value] = implicit_function(X, para)
        % transform pose parameters into R matrix and t vector
        R = eul2rotm(para(6 : 8));
        t = para(9 : 11);
        % align the point cloud to the superquadrics coordinate
        X_c = R' * X - R' * t';       
        implicit_value = (((X_c(1, :) / para(3)) .^ (2)) .^ (1 / para(2)) + ...
            ((X_c(2, :) / para(4)) .^ (2)) .^ (1 / para(2))) .^ (para(2) / para(1)) + ...
            ((X_c(3, :) / para(5)) .^ (2)) .^ (1 / para(1));
    end
...
function [para] = parseInputArgs(point, varargin)
    ...
    parser.addParameter('IncludeAllInliers', defaults.IncludeAllInliers, @islogical);
    parser.addParameter('InlierThreshold', defaults.InlierThreshold, ...
        @(x)validateattributes(x, {'single', 'double'}, {'real', 'nonsparse', 'nonempty', 'scalar', 'nonnan', 'finite', 'nonnegative' , '>=', 0, '<=', 1}));
