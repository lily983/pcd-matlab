...
ply_prefix = '../../data/';
files = dir( [ply_prefix, '/*.ply'] );

num_files = size(files, 1);

sq_out_prefix = '../../config/';

%% Fitting superquadrics to convex polyhedra
disp('Fitting superquadrics to .ply...')
...
for i = 1:num_files
    ply_file_name = [files(i).folder, '/', files(i).name];
    
    % Generate random convex polyhedra
    point_cloud = double( pcread(ply_file_name).Location' );
...
csvwrite([sq_out_prefix, 'obstacle_real_ems.csv'], sq_shape_list);