function [mf, Sigmaf] = get_lower_bounding_ellip(s1, s2)
% This function gets the John lower bounding ellip for the Minkowski sum of s1 and s2
...
% Sigmaf =  (Q1^2 + 2.*geometricMean(Q1^2, Q2^2) + Q2^2)^0.5;
Sigmaf =  (Q1 + 2*geometricMean(Q1, Q2) + Q2);

mf = zeros(dimension, 1);
end

% Definition 5.4 of paper
%Closed-Form Parametric Equation for the Minkowski Sum of $ m $ Ellipsoids in $\mathbb {R}^ N $ and Associated Volume Bounds.
function M = geometricMean(P, Q)
M = P^0.5 * (P^0.5 \ Q / P^0.5)^0.5 * P^0.5;
end