classdef SuperQuadrics < handle
    ...
    %    val{8}: [Optional] Indicator whether use almost uniform sampling
    %            on surface, default = false
    %
    %  Author:
    ...
    methods
        ...
        function obj = SuperQuadrics(val)
            ...
            elseif length(val{5}) ~= 4
                ...
                if length(val) == 8
                    if strcmp(val{8}, 'uniform')
                        [obj.eta, obj.omega] = uniform_sample_superquadrics(obj,3);
                    end
                end
            end
        end
        ...
        %% Get Tangent vectors at surface points
        function [t1_t, t2_t] = GetTangents(obj)
            % GetGradients Get the surface gradients in global frame
            %
            %  Outputs:
            %    t1_t, t2_t: Surface tangents in global frame, 3xN vector
            
            [t1, t2] = obj.GetTangentsCanonical();
            t1_t = quat2rotm(obj.q) * t1;
            t2_t = quat2rotm(obj.q) * t2;
        end
        
        function [t1, t2] = GetTangentsCanonical(obj)
            % GetTangentsCanonical Get tangent vectors (un-normalized) on
            % boundary, in canonical form
            %
            %  Outputs:
            %    t1, t2: Surface tangents in body frame, 3xN vector
            
            [t1_x, t1_y, t1_z, t2_x, t2_y, t2_z] = obj.GetSurfTangentsCanonical();
            
            % Reshape to 3xN array
            [mm, nn] = size(t1_x);
            t1 = nan(3,mm*nn);
            t1(1,:) = reshape(t1_x, 1, mm*nn);
            t1(2,:) = reshape(t1_y, 1, mm*nn);
            t1(3,:) = reshape(t1_z, 1, mm*nn);
            
            t2 = nan(3,mm*nn);
            t2(1,:) = reshape(t2_x, 1, mm*nn);
            t2(2,:) = reshape(t2_y, 1, mm*nn);
            t2(3,:) = reshape(t2_z, 1, mm*nn);
        end
        
        function [t1_x, t1_y, t1_z, t2_x, t2_y, t2_z] = GetSurfTangentsCanonical(obj)
            t1_x = -(obj.a(1)*obj.eps(1)) .*...
                eps_fun(abs(cos(obj.eta)), obj.eps(1)-1) .*...
                eps_fun(cos(obj.omega), obj.eps(2)) .* sin(obj.eta);
            t1_y = -(obj.a(2)*obj.eps(1)) .*...
                eps_fun(abs(cos(obj.eta)), obj.eps(1)-1) .*...
                eps_fun(sin(obj.omega), obj.eps(2)) .* sin(obj.eta);
            t1_z = (obj.a(3)*obj.eps(1)) .*...
                eps_fun(abs(sin(obj.eta)), obj.eps(1)-1) .* cos(obj.eta);
            
            t2_x = -(obj.a(1)*obj.eps(2)) .*...
                eps_fun(cos(obj.eta), obj.eps(1)) .*...
                eps_fun(abs(cos(obj.omega)), obj.eps(2)-1) .* sin(obj.omega);
            t2_y = (obj.a(2)*obj.eps(2)) .*...
                eps_fun(cos(obj.eta), obj.eps(1)) .*...
                eps_fun(abs(sin(obj.omega)), obj.eps(2)-1) .* cos(obj.omega);
            t2_z = zeros(size(obj.eta));
        end
        
        function [t1, t2] = GetTangentFromSpherical(obj, theta)
            t1 = nan(3,size(theta,1));
            
            t1(1,:) = -(obj.a(1)*obj.eps(1)) .*...
                eps_fun(abs(cos(theta(1,:))), obj.eps(1)-1) .*...
                eps_fun(cos(theta(2,:)), obj.eps(2)) .* sin(theta(1,:));
            t1(2,:) = -(obj.a(2)*obj.eps(1)) .*...
                eps_fun(abs(cos(theta(1,:))), obj.eps(1)-1) .*...
                eps_fun(sin(theta(2,:)), obj.eps(2)) .* sin(theta(1,:));
            t1(3,:) = (obj.a(3)*obj.eps(1)) .*...
                eps_fun(abs(sin(theta(1,:))), obj.eps(1)-1) .* cos(theta(1,:));
            
            t2 = nan(3,size(theta,1));
            t2(1,:) = -(obj.a(1)*obj.eps(2)) .*...
                eps_fun(cos(theta(1,:)), obj.eps(1)) .*...
                eps_fun(abs(cos(theta(2,:))), obj.eps(2)-1) .* sin(theta(2,:));
            t2(2,:) = (obj.a(2)*obj.eps(2)) .*...
                eps_fun(cos(theta(1,:)), obj.eps(1)) .*...
                eps_fun(abs(sin(theta(2,:))), obj.eps(2)-1) .* cos(theta(2,:));
            t2(3,:) = zeros(1,size(theta,1));
        end
