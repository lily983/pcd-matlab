function [prob, time] = enlargedSQlinearChanceConstraint.m(sub1, sub2, xx, Sigmax, isplot)
% enlargedSQlinearChanceConstraint: This function is modified for two EnlargedSuperquadrics based on methods using linear chance constraint (LCC)
...
% Inputs:
% sub, sub2: enlarged superquadrics that encapsulate the surface points of
% s1 s2 copies at observed rotations (upper bound for SO(3) errors)
% xx: center of the relative position error distribution
% Sigma: covariance matrix of the relative position error distribution
% Outputs:
% prob: PCD value
